...
class WolfSmartSet {
    ...
    async adminGetDevicelist(user, password) {
        ...
        this.USER = origUser;
        this.PASS = origPass;
        this._setAuthData(origAuth, true);
     ...
     */
    async _endsession() {
        this.adapter.log.debug(`_endsession() starting ...`);

        if (!this.auth || !this.auth.id_token) {
            this.adapter.log.debug(`_endsession(): id_token already removed ...`);
        ...
        if (typeof endSessionUrl == 'undefined') {
            this.adapter.log.warn(`_endsession(): endSession endpoint not found!`);
            this._removeAuthData();
            return;
        }
    ...
    async _createSession() {
        ...
        try {
            ...
            //     TE: 'trailers',
            // };
            const headers = await this._generateHeader('application/json; charset=utf-8');
