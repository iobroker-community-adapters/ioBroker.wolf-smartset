...
class WolfSmartSet {
    ...
    _removeAuthData() {
        ...
        try {
            ...
            this.adapter.log.warn(`Could not remove auth data cache file(${this.authFile}): ${error.message}`);
        }
    }
     ...
     */
    async _getAuthToken() {
        this.adapter.log.info(`_getAuthToken(): starting ...`);
    ...
    async _refreshAuthToken() {
        this.adapter.log.info(`_refreshAuthToken(): starting ...`);
        this.refreshTimeout && clearTimeout(this.refreshTimeout);

        // if no auth data available, start a fresh authentication
        if (!this.auth || !this.auth.id_token) {
            this._setAuthData(await this._getAuthToken(), false);
            return;
        }
        ...
        try {
            ...
            if (params.code) {
                ...
                this._setAuthData(tokenSet.data, false);
            } else if (params.error) {
    ...
    async _destroyAuthToken() {
        ...
        let endSessionUrl = this.openIdClient && this.openIdClient.endSessionUrl();
    ...
    async init() {
        ...
        if (this.auth && this.auth.access_token) {
            ...
            if (typeof this.UserInfo != 'undefined') {
                if (this.adapter.config.pollInterval < this.UserInfo.MaxSystemBusSamplingRateSec) {
                    this.adapter.log.warn(
                        `Adapter poll interval (${this.adapter.config.pollInterval.toString()} s) is less than min poll interval (${this.UserInfo.MaxSystemBusSamplingRateSec.toString()} s) permitted by Wolf: consider changing adapter settings!`,
                    );
                }
            }
            ...
            this.LastAccess = null; // reset last access to get full value list after re-login
            this.FullPull = this.adapter.config.doFullPull;
     ...
     *
     * @param username - Wolf login username
     * @param password - Wolf login password
     */
    async adminGetDevicelist(username, password) {
        let system;
        try {
            this.USER = username;
            this.PASS = password;
            this._setAuthData(await this._getAuthToken(), true);
    ...
    async getValList(GatewayId, SystemId, BundleValuesList) {
        const bundleId = this.FullPull ? 500 : this.isExpertSession ? 3100 : 1000;
        const valueIdList = this.FullPull
            ? BundleValuesList[0]
        ...
        if (this.LastAccess == null) {
            this.adapter.log.info(`API GET VAL LIST: asking for full value list (${valueIdList.length} values)`);
        }

        if (this.adapter.config.doApiProfile) {
            this.adapter.setState('info_api.poll_req_bundle_id', { val: bundleId, ack: true });
        ...
        try {
            ...
            if (this.adapter.config.doApiProfile) {
                ...
                if (paramsNoValue != '') {
                    this.adapter.log.warn(`API GET VAL LIST: got no value for: ${paramsNoValue}`);
                }
            }
            return response.data;
        } catch (error) {
