    ...
    async getValList(GatewayId, SystemId, BundleValuesList) {
        ...
        try {
            ...
            if (this.adapter.config.doApiProfile) {
                ...
                if (paramsNoValue != '') {
                    this.adapter.log.warn(`API GET VAL LIST: go no value for: ${paramsNoValue}`);
                }
            }
            return response.data;
        } catch (error) {
    ...
    async _endsession() {
        ...
        try {
            ...
            if (endsessionResponse.status != 200) {
                this.adapter.log.warn(`API ENDSESSION WARNING: unexpected response: ${endsessionResponse.statusText} (${endsessionResponse.statusText})`);
            } else {
