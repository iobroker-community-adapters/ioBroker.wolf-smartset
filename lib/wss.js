...
class WolfSmartSet {
	...
	async adminGetDevicelist(user, password) {
		...
		this.log.info('adminGetDevicelist');
		let system;
		try {
			this.USER = user;
			this.PASS = password;
	...
	async getGUIDescription(gatewayid, id) {
		this.log.info('getGUIDescription');
		const headers = await this._generateHeader('');
		try {
	...
	generateDateTimeString() {
		this.log.info('generateDateTimeString');
		const date = new Date();
