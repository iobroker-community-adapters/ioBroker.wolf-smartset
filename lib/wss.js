...
class WolfSmartSet {
    ...
    async init() {
        ...
        if (this.auth && this.auth.access_token) {
            ...
            // _getUserCapabilites() must be called after _createSession(), do not use await here
            ...
            //         this.adapter.log.warning(
            //             `Adapter poll interval (${this.adapter.config.pingInterval}s) is less than user's permitted min poll interval (${this.UserInfo.MaxSystemBusSamplingRateSec}): consider changing adapter settings!`,
            //         );
            //     }
            // }
     ...
     *      "Email":"software@messmer-online.de",
    ...
    async _getUserCapabilities() {
        ...
        this.adapter.log.debug(
            `API GET USERINFO: calling: ${_API_STRING['WEBSERVER']}${_API_STRING['PATH_PORTAL_USERINFO']}`,
        );
        try {
     ...
     * @param GatewayId - Gateway Id as received by _getSystemList()
     * @param SystemId - SystemId as received by _getSystemList()
     * @param valList - list of values of interest
     */
    async getValList(GatewayId, SystemId, valList) {
        const headers = await this._generateHeader('application/json');
        const payload = {
            BundleId: 1000,
            IsSubBundle: false,
