...
class WolfSmartSet {
    ...
    async openIdInit() {
        ...
        this.openIdIssuer = await Issuer.discover(
            _API_STRING["WEBSERVER"] + _API_STRING["PATH_OPENID_CFG"] ,
        );
    ...
    async init() {
        ...
        if (this.auth && this.auth.access_token) {
            ...
            this.adapter.log.debug("Fetch Expert Values: " + this.adapter.config.doExpert.toString() + ", Expert PW: " + (this.adapter.config.expertPassword.length > 0 ? '****' : 'not set'));
    ...
    async _expertLogin(expertPassword) {
        ...
        try {
            const response = await axios.get(_API_STRING["WEBSERVER"] + _API_STRING["PATH_EXPERT_LOGIN"] + '?' + _API_STRING["PARAM_EXPERT_PASSWORD"] + expertPassword, {
                headers: headers,
            });
            if (this.destroyed) {
                return;
    ...
    async getGUIDescription(gatewayid, id) {
        const headers = await this._generateHeader('');
        try {
            const response = await axios.get(_API_STRING["WEBSERVER"] + _API_STRING["PATH_GET_GUI_DESC_FOR_GW"] + '?' + _API_STRING["PARAM_GATEWAY_ID"] + gatewayid + '&' +_API_STRING["PARAM_SYSTEM_ID"] + id, {
    ...
    async setParameter(GatewayId, SystemId, Paramarray) {
        ...
        this.adapter.log.debug(`SEND VALUE ${_API_STRING["WEBSERVER"]}${_API_STRING["PATH_SET_PARAM_VALS"]} : ${JSON.stringify(payload)}`);
        try {
    ...
    async getSystemState(sysId) {
        ...
        try {
            const response = await axios.get(_API_STRING["WEBSERVER"] + _API_STRING["PATH_GET_SYSTEM_STATE"] + sysId, {
                headers: headers,
            });
            if (this.destroyed) {
                return;
