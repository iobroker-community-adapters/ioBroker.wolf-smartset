...
class WolfSmartSet {
	constructor(username, password, adapter) {
		...
		try {
			...
			this.adapter.log.error(`Error creating directory: ${error.message}`);
		}
	...
	async openIdInit() {
		...
		try {
			...
			this.adapter.log.error(`Error reading auth.json: ${error.message}`);
		}
	}
	...
	setAuthData(auth, doNotSave) {
		...
		if (!doNotSave) {
			try {
				...
				this.adapter.log.error(`Error writing auth.json: ${error.message}`);
			}
		}
	}
	...
	async init() {
		...
		this.adapter.log.debug('Init done!');
	...
	async adminGetDevicelist(user, password) {
		...
		try {
			...
			this.adapter.log.error('API GET ADMIN DEVICE LIST ERROR: ' + error.message);
		}
	...
	async _getAuthToken() {
		...
		try {
			...
			this.adapter.log.error('API GET AUTH TOKEN ERROR: ' + error.message);
	...
	async _getSystemList() {
		...
		try {
			...
			this.adapter.log.error('API GET SYSTEM LIST ERROR: ' + error.message);
		}
	}
	async getGUIDescription(gatewayid, id) {
		...
		try {
			...
			this.adapter.log.error('API GET GUI DESC. ERROR: ' + error.message);
			console.log(error);
		}
	}
	async getValList(gateway, sysId, valList) {
		...
		try {
			...
			this.adapter.log.error('API GET VAL LIST ERROR: ' + error.message);
		}
	}
	async stop() {
		this.UpdateTimeout && clearTimeout(this.UpdateTimeout);
	...
	async _refresh() {
		...
		try {
			...
			this.adapter.log.error('API REFRESH TOKEN ERROR: ' + error.message);
			this.adapter.log.error(error.stack);
			this.auth = {};
		}
	}
	...
	async setParameter(GatewayId, SystemId, Paramarray) {
		...
		try {
			...
			this.adapter.log.error('SEND VALUE ERROR: ' + error.message);
		}
	}
	...
	async _sessionUpdate() {
		...
		try {
			...
			this.adapter.log.error('SESSION UPDATE ERROR: ' + error.message);

			if (this.SessioID && error.response && error.response.status === 401) {
				this.SessioID = this._createSession();
	...
	async getSystemState(sysId) {
		...
		try {
			...
			this.adapter.log.error('API GET SYSTEM STATE: ' + error.message);
		}
	}
	...
	async _createSession() {
		...
		try {
			...
			*/
			...
			this.adapter.log.error('API CREATE SESSION ERROR: ' + error.message);
			this.adapter.log.error(error.stack);
		}
	}
