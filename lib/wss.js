...
class WolfSmartSet {
	...
	async openIdInit() {
		this.adapter.log.debug('openIdInit');
		...
		// console.log(util.inspect(this.openIdIssuer, {depth: null}));
	...
	setAuthData(auth, doNotSave) {
		this.adapter.log.debug('setAuthData');
		if (!auth) return;
	...
	async adminGetDevicelist(user, password) {
		...
		this.adapter.log.debug('adminGetDevicelist');
		let system;
		try {
			this.USER = user;
			this.PASS = password;
	...
	async _getAuthToken() {
		this.adapter.log.debug('_getAuthToken');
	...
	async _getSystemList() {
		this.adapter.log.debug('_getSystemList');
	...
	async getGUIDescription(gatewayid, id) {
		this.adapter.log.debug('getGUIDescription');
		const headers = await this._generateHeader('');
		try {
	...
	async getValList(gateway, sysId, valList) {
		this.adapter.log.debug('getValList');
	...
	async _refresh() {
		this.adapter.log.debug('_refresh');
		this.refreshTimeout && clearTimeout(this.refreshTimeout);
	...
	async setParameter(GatewayId, SystemId, Paramarray) {
		this.adapter.log.debug('setParameter');
		const payload = {
			SessionId: this.SessioID,
			BundleId: 1000,
			GatewayId: GatewayId,
	...
	async _sessionUpdate() {
		this.adapter.log.debug('_sessionUpdate');
		this.UpdateTimeout && clearTimeout(this.UpdateTimeout);
	...
	async getSystemState(sysId) {
		this.adapter.log.debug('getSystemState');
		const headers = await this._generateHeader('application/json');
