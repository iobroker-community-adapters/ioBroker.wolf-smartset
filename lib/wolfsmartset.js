...
class WolfSmartSet {
    ...
    async _getAuthToken() {
        ...
        try {
            ...
            setTimeout(() => {this._refresh();}, response.data.expires_in * 1000)
    ...
    async getGUIDescription(gatewayid, id) {
        let headers = await this._generateHeader('');
        try {
            const response = await axios.get(apiPoint + '/portal/api/portal/GetGuiDescriptionForGateway?GatewayId=' + gatewayid + '&SystemId=' + id, {
                headers: headers
            });
    ...
    async getValList(gateway, sysId, valList) {
        ...
        try {
            const response = await axios.post(apiPoint + '/portal/api/portal/GetParameterValues', payload, {
                headers: headers
            });
            this.adapter.log.debug('API GET GUI DESK.: ' + JSON.stringify(response.data));
    ...
    async _sessionUpdate() {
        ...
        try {
            const response = await axios.post(apiPoint + '/portal/api/portal/UpdateSession', payload, {
                headers: headers
            });
